...
A = mutate(A, y=as.factor(A$y)) %>%
  ...
  mutate(month=as.factor(A$month))
str(A)

#plot each feature
ggplot(data = A, mapping = aes(x = age, y = previous, color = y)) +
  geom_point()

ggplot(data = A, mapping = aes(x = y, y = campaign)) +
  geom_boxplot()
...
#Select features
A = select(A, y, duration, education, contact, default, housing, loan, marital, poutcome, job, month)
head(A)
str(A)
...
#Best one
lr = glmnet(y ~ . -month-default, data = ATrain, family = "binomial")
#Accuracy
prediction = predict(lr, ATest, type = "class", s = 0.01)
confusionMatrix(prediction, ATest$y)
